...
def initialization_cross(config_source, config_target, train=False, test=False):
    print("Initialzing...")
    WORK_PATH = config_source['WORK_PATH']
    os.chdir(WORK_PATH)
    os.environ["CUDA_VISIBLE_DEVICES"] = config_source["CUDA_VISIBLE_DEVICES"]
    train_source, _ = initialize_data(config_source, train, test)
    _, test_target = initialize_data(config_target, train, test)
    return initialize_model(config_source, train_source, test_target)
